...
**As of:** 2026-04-23  
**Version:** v3.12.0-reports-cache (Phase 14 Complete, pending merge)  
...
**Stability Review Note:** 2026-04-22 — full regression run completed with local temp dir override (`TMPDIR=/tmp TEMP=/tmp TMP=/tmp`, `523 passed, 1 skipped`). Review identified four stability gaps to track in Task.md: (1) SIEM producer/consumer pipeline wiring is incomplete in runtime path, (2) dispatch rows can remain permanently `pending` when payload build returns `None`, (3) `cron_expr` evaluation forces UTC and can drift for non-UTC schedule timezones, (4) scheduler cache/SIEM registration exceptions are swallowed without logging.
**i18n Review Note:** 2026-04-22 — `python scripts/audit_i18n_usage.py` still reports `0 findings`, but leakage risk remains because parts of Web GUI text are hardcoded in JS-generated HTML (not key-based), and `src/i18n.py` still allows humanize/auto-translate fallback for missing keys. Result: CI can pass while end users still see mixed EN/ZH strings.
**i18n Gate Update:** 2026-04-22 — first mitigation landed: all `_translations[key] || 'English...'` fallbacks in `src/static/js/*.js` were replaced with strict `_t(key)` lookup (missing key now renders `[MISSING:key]` instead of silently falling back to English), and i18n audit added Category **H** to fail CI on any future JS/HTML fallback literal pattern.
**i18n Verification Note:** 2026-04-22 — post-change validation passed: JS syntax check (`node --check` on modified UI files), `python scripts/audit_i18n_usage.py` (`A–H = 0 findings`), and targeted regression tests (`TMPDIR=/tmp TEMP=/tmp TMP=/tmp pytest -q --basetemp=/tmp/pytest-illumio tests/test_i18n_audit.py tests/test_i18n_quality.py tests/test_gui_security.py` → `42 passed`).
**i18n Hardening Note:** 2026-04-22 — localization source upgraded from heuristic-only mode to explicit dual-file mode: new `src/i18n_zh_TW.json` mirrors `src/i18n_en.json` (1583 keys), `src/i18n.py` now prefers explicit zh table and applies strict missing-key markers for tracked UI/CLI/report/alert prefixes, and dynamic `event_label_*`/`cat_*` keys remain humanized fallback-safe for event catalog UX.
**i18n Audit Expansion:** 2026-04-22 — audit script now has Category **I** (EN↔zh_TW tracked-key parity) and treats `[MISSING:key]` as a hard failure. Latest run: `A–I = 0 findings`.
**Cross-surface Verification:** 2026-04-22 — integrated i18n regression across Web UI/CLI/report/message-alert paths passed: `71 passed` (`tests/test_i18n_audit.py`, `tests/test_i18n_quality.py`, `tests/test_gui_security.py`, `tests/test_report_generator.py`, `tests/test_policy_usage_report.py`, `tests/test_audit.py`, `tests/test_cli_report_commands.py`, `tests/test_main_menu.py`, `tests/test_event_monitoring.py`).
**i18n Cleanup Note:** 2026-04-23 — second-pass Web UI cleanup completed for high-risk runtime JS paths (`actions.js`, `events.js`, `module-log.js`, `quarantine.js`, `rule-scheduler.js`, `settings.js`, `dashboard.js`): fallback literals removed, runtime toasts/errors/scheduler logs switched to `_t(...)`, and 83 new GUI keys were added explicitly to both `i18n_en.json` and `i18n_zh_TW.json` (key count now 1666 each). Validation remains green: JS syntax check pass, `A–I = 0 findings`, cross-surface regression `71 passed`.
**i18n Cleanup Note (Round 3):** 2026-04-23 — dashboard i18n hardening continued: `dashboard.js` + `dashboard_v2.js` now normalize report payload aliases (`Port/Flow Count/Bytes Total/Src IP/Dst IP` and snake_case variants) via a shared lookup helper to avoid mixed-language/empty-cell regressions, policy-usage summary section keys were added explicitly (including relative-time keys), and zh labels were adjusted to keep domain terms while avoiding placeholder leakage. Root-cause guardrail was also fixed: both `src/i18n.py::_discover_keys()` and `scripts/audit_i18n_usage.py` now scan `_t(...)` in addition to `t(...)`, preventing future false-green i18n audits. Validation: JS syntax check passed, `python scripts/audit_i18n_usage.py` (`A–I = 0`), targeted regression (`tests/test_i18n_audit.py`, `tests/test_i18n_quality.py`, `tests/test_gui_dashboard_plotly_endpoint.py`) = `15 passed`.
**i18n Completion Note:** 2026-04-23 — `P1c` final sweep completed for remaining UI literals in templates + runtime dashboard cards (skip-link, report-generation source labels, apply/query actions, throttle labels, floating quarantine action bar, card ON/OFF/NONE state text, uncovered-percentage text). Intentional English domain term `Workloads` was explicitly whitelisted in `_ZH_EXPLICIT` to avoid false-positive leak flags while preserving glossary policy. Validation: `python scripts/audit_i18n_usage.py` (`A–I = 0`), i18n/UI regression (`15 passed`), and report/alert regression (`19 passed`).
**SIEM Preview Positioning Note:** 2026-04-23 — SIEM forwarding is now explicitly positioned as Preview (not GA). For compatibility, currently enabled SIEM deployments remain runnable; startup/runtime warning logs are emitted in daemon/GUI/scheduler/siem-CLI paths, docs now carry Preview warnings, and scheduler cache/SIEM registration failures are no longer silently swallowed (exceptions are logged).
