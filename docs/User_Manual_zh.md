...
| `web_gui.password` | string | `illumio` | **首次登入後請立即變更。** GUI 儲存 Argon2 雜湊值 — 範例中的明文密碼僅在首次使用前有效。 |
...
| 解除安裝後重新安裝，Web GUI 登入失敗 | 舊設定中保留的 `web_gui.password_hash` | 雜湊值有效 — 密碼未變更。使用舊密碼登入。 |
