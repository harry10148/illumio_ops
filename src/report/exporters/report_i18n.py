...
STRINGS: _StringMap = _StringMap({
    ...
    "rpt_pu_sec_deny": _entry("4 Deny Rule Effectiveness", "4 Deny Rule 有效性分析"),
    "rpt_pu_deny_total": _entry("Total Deny Rules", "Deny Rule 總數"),
    "rpt_pu_deny_hit": _entry("Hit", "已命中"),
    "rpt_pu_deny_unused": _entry("Unused", "未使用"),
    "rpt_pu_deny_hit_rate": _entry("Hit Rate", "命中率"),
    "rpt_pu_override_deny": _entry("Override Deny Rules:", "Override Deny Rules："),
    "rpt_pu_deny_detail": _entry("Deny Rule Details", "Deny Rule 明細"),
    "rpt_pu_no_deny": _entry("No deny rules found in the active policy.", "目前 Active Policy 中沒有 Deny Rule。"),
...
for rule_id, zh_name in {
    ...
    "L002": "網路探索協定暴露",
    "L003": "資料庫連接埠從多個 App 層可達",
    "L004": "跨環境資料庫存取",
    "L005": "身份驗證基礎架構大範圍暴露",
    "L006": "高擴散半徑 Lateral 路徑（Graph BFS）",
    "L007": "Unmanaged 主機存取關鍵服務",
    "L008": "Lateral 連接埠處於 Test Mode — Policy 尚未 Enforce",
    "L009": "資料外洩模式 — 流出至 Unmanaged 主機",
    "L010": "跨環境 Lateral 連接埠存取 — 邊界突破",
}.items():
    en_name = {
