...
| 功能 | 說明 |
|:---|:---|
| **三種執行模式** | 背景守護程序 (`--monitor`)、互動式 CLI（根選單 6 項，各含子選單）、或 Flask 驅動的 **Web GUI** (`--gui`) |
...
| **規則排程器** | 依時間區間自動啟用/停用 PCE 規則；**三層 Draft 安全防護**（CLI/GUI/引擎）防止未 Provision 規則被意外派送 |
| **工作負載隔離** | 透過 Quarantine Label（Mild/Moderate/Severe）隔離遭入侵主機；支援 IP/CIDR/子網段搜尋 |
